...
public final class AccountRegistrationService {
    ...
    private Result<String, BackendErrorResponse> registerAccountOnBackEnd(@NonNull AccountRegistrationReq req) {
        return backendClientInterface.registerAccount(req);
    }

    private Result<Account, String> getAccountInfo(@NonNull CustomerEntity customer) {
    ...
    private Result<Account, String> registrationFail(@NonNull Result<String, BackendErrorResponse> register) {
        return register.getError()
            .map(error -> Result.<Account, String>error(error.message()))
            .orElse(Result.error(UNKNOWN_ERROR));
    }

    private BigDecimal addBonusToBalance(@NonNull BigDecimal balance) {
